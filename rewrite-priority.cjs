...
const startIndex = code.indexOf("app.post('/api/shiprocket/order/priority-book'");
const endIndex = code.indexOf("app.post('/api/shiprocket/reconcile-shipment'");
...
// Rewind endIndex to previous comment if any
const beforeReconcile = code.lastIndexOf("// API Route: ", endIndex);
const finalEndIndex = beforeReconcile > startIndex ? beforeReconcile : endIndex;
...
const newCode = code.slice(0, startIndex) + functionBody + code.slice(finalEndIndex);
