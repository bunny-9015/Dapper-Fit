...
const regex = /\/\/ API route: Get current products[\s\S]*?res\.json\(\{ replacements: sorted \}\);\n\}\);/;

const properPart = `// API route: Get current products
...
const newCode = code.replace(regex, properPart);

if (newCode !== code) {
  fs.writeFileSync('server.ts', newCode);
  console.log('Reconstructed broken part using regex successfully!');
} else {
